        ...
        <form method="POST" action="{{ route('personal.store') }}" enctype="multipart/form-data">
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label for="división" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            División <span class="text-red-500">*</span>
                        </label>
                        <select name="division" id="division" required
                                class="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-blue-500 dark:focus:border-blue-400 text-gray-900 dark:text-white transition-colors duration-200">
                            <option value="">Seleccionar...</option>
                            <option value="Operativa" {{ old('división') === 'Operativa' ? 'selected' : '' }}>Operativa</option>
                            <option value="Administrativa" {{ old('división') === 'Administrativa' ? 'selected' : '' }}>Administrativa</option>
                        </select>
                        @error('división')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
