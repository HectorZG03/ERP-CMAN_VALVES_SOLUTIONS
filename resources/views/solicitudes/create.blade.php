...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold text-gray-900">Nueva Solicitud de Material</h1>
        ...
        <form method="POST" action="{{ route('solicitudes.store') }}" id="solicitudForm">
            ...
            <div class="mb-8">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-900">Productos Solicitados</h3>
                    <span class="text-sm text-gray-500" id="contador-productos">0 productos agregados</span>
                </div>
                ...
                <div id="mensaje-inicial" class="text-center py-8 text-gray-500">
            ...
            <div class="bg-blue-50 p-4 rounded-lg mb-6">
                <h3 class="text-lg font-medium text-blue-900 mb-2">Proceso de Aprobación</h3>
            ...
            <div id="resumen-solicitud" class="bg-gray-50 p-4 rounded-lg mb-6 hidden">
                <h3 class="text-lg font-medium text-gray-900 mb-3">Resumen de la Solicitud</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div class="text-center">
                        <div class="text-2xl font-bold text-blue-600" id="total-productos">0</div>
                        <div class="text-gray-600">Productos Diferentes</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-green-600" id="total-unidades">0</div>
                        <div class="text-gray-600">Total Unidades</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-purple-600" id="valor-estimado">$0.00</div>
                        <div class="text-gray-600">Valor Estimado</div>
                    </div>
                </div>
            </div>
...
<template id="template-producto">
    <div class="producto-item bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
            <div class="flex-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                        <p class="text-sm text-gray-600">Categoría: <span class="producto-categoria"></span></p>
                        <p class="text-sm text-gray-600">Medida: <span class="producto-medida"></span></p>
                    </div>
                    
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Cantidad</label>
                        <input type="number" 
                               class="cantidad-input w-full text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" 
                               min="1" 
                               value="1"
                    ...
                    <div class="text-center">
                        ...
                        <p class="font-semibold text-green-600 producto-stock"></p>
                    </div>
                    
                    <div class="text-center">
                        <p class="text-xs text-gray-600">Precio Unit.</p>
                        <p class="font-semibold text-blue-600 producto-precio">$0.00</p>
                    </div>
                </div>
            </div>
        </div>
...
function mostrarResultados(productos) {
    ...
    if (productos.length === 0) {
        resultados.innerHTML = '<div class="p-3 text-gray-500 text-center">No se encontraron productos</div>';
    ...
    productos.forEach(producto => {
        ...
        const claseDisabled = yaAgregado ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50 cursor-pointer';
        
        html += `
            <div class="p-3 border-b border-gray-100 ${claseDisabled}" 
                 ${!yaAgregado ? `onclick="agregarProducto(${JSON.stringify(producto).replace(/"/g, '&quot;')})"` : ''}>
                <div class="flex justify-between items-center">
                    <div>
                        <div class="font-medium text-gray-900">${producto.nombre_producto}</div>
                        <div class="text-sm text-gray-600">${producto.categoria} - ${producto.medida}</div>
                    </div>
                    <div class="text-right">
                        <div class="text-sm font-medium ${producto.existencia > 10 ? 'text-green-600' : 'text-yellow-600'}">
                            Stock: ${producto.existencia}
                        </div>
                        ${yaAgregado ? '<span class="text-xs text-gray-400">Ya agregado</span>' : ''}
                    </div>
                </div>
            </div>
        `;
