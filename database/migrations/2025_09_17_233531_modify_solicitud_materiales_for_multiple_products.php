...
return new class extends Migration
{
    ...
    {
        // Crear tabla de detalles para múltiples productos por solicitud
        Schema::create('solicitud_material_detalles', function (Blueprint $table) {
            ...
            $table->decimal('precio_unitario', 10, 2)->nullable(); // Para llevar registro del precio al momento
            $table->timestamps();
        });

        // Modificar tabla principal para remover campos que ahora van en detalles
        Schema::table('solicitud_materiales', function (Blueprint $table) {
            $table->dropForeign(['inventario_id']);
            $table->dropColumn(['inventario_id', 'cantidad_solicitada']);
        });
    }
